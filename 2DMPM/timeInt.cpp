...
void UVF::advance(patch&pch){
   for(int i=0;i<pch.Npart();++i){pch.pJ[i]=getStress(pch,pch.pF[i],pch.pVS[i]);pch.pVS[i]*=pch.pV[i]*pch.pJ[i];}
...
void cenDif::advance(patch&pch){
   ...
   for(int i=0;i<pch.Nnode();++i)
	pch.gv[i] = pch.gw[i]/pch.gm[i];
   
   // Divide force by mass to get acceleration
   for(int i=0;i<pch.Nnode();++i)
	pch.ga[i] = (pch.gfe[i]+pch.gfi[i]) / pch.gm[i];
   ...
   for(int i=0;i<pch.Npart();++i)
	pch.pv[i] += pch.pvI[i] * pch.dt;
   
   // Advance the particle position
   for(int i=0;i<pch.Npart();++i)
	pch.px[i] += pch.pxI[i]*pch.dt;
   
   // Advance the deformation gradient
   for(int i=0;i<pch.Npart();++i)
	pch.pF[i] += pch.pGv[i].inner(pch.pF[i]) * pch.dt;
}

timeIntSC&makeTimeInt(string s,shapeSC&sh,patch&){
   if(s=="UVF"){return*new UVF::UVF(sh);}
   if(s=="CD" ){return*new cenDif::cenDif(sh);}
